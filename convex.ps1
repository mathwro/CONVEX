...
if (Get-InstalledModule -name AzureRM -ErrorAction SilentlyContinue) {Write-Error "AzureRM cannot be installed for CONVEX to run. Please uninstall AzureRM and install the new Azure Module, AzureAD, Azure CLI, and Azure Function Core Tools."}

# Make sure the three required modules are installed
if (-Not (Get-InstalledModule -name Azure -ErrorAction SilentlyContinue)) {Write-Error "Azure Module needs be installed for CONVEX to run. Please uninstall AzureRM and install the new Azure Module, AzureAD, Azure CLI, and Azure Function Core Tools."}
if (-Not (Get-InstalledModule -name AzureAD -ErrorAction SilentlyContinue)) {Write-Error "AzureAD Module needs be installed for CONVEX to run. Please uninstall AzureRM and install the new Azure Module, AzureAD, Azure CLI, and Azure Function Core Tools."}
...
do {
    ...
    #$input1 = "*" + $prompt1 + "*"
    $SubOne = $allSubs | Where-Object id -CLike $prompt1
    $s1 = $subIds.Match($SubOne.Id)
